...
An optional `render.worker.yaml` specifies one Docker web service, a 10 GB disk, health checks, Kimi and disabled automatic deploys. It is a deployment starting point; **the AMD64 image and non-root Chromium sandbox passed a free Render compatibility probe**. Its compute and disk are paid resources. Review them before provisioning. The probe returned browser readiness with sandboxing enabled on the actual Render host. Full generation still requires the paid worker configuration and persistent disk. The [Render Blueprint reference](https://render.com/docs/blueprint-spec) describes these settings; [persistent disk documentation](https://render.com/docs/disks) covers lifecycle and backup behavior.
...
- `node --test scripts/test-reconstruction-browser.mjs`: verifies separate animation properties, reduced motion, screenshot viewport resets, and real mobile resizing.
...
All 79 automated tests passed (63 generator/builder/reconstruction checks, 15 worker/reconnection checks and one separate-process browser integration test), along with lint and the frontend build. The existing large-chunk build warning remains. The local preflight passed with private Blob, Chrome and configured NVIDIA Kimi. The separate-process integration test passed, including a mid-job restart. Docker and Colima are installed for verification. The Linux ARM64 image built and its production preflight passed with a non-root, sandboxed Chromium process, private Blob storage and a writable named volume at 1 CPU / 2 GB RAM. Both ARM64 and AMD64 images built. AMD64 Chromium could not run under the local ARM emulator because SSE3 was unavailable, so the same image was tested on Render’s real AMD64 host instead; that browser probe passed. A fresh Momentum Fit gym capture exposed and led to fixes for separate opacity/transform animations, invalid model inspection requests, and Chromium’s temporary 1-pixel screenshot viewport. The corrected Linux comparison passed at 1440/768/390 px (5.30%, 3.48%, 2.77% pixel differences; no missing images or overflow). Full Kimi personalization completed, passed desktop/tablet/phone checks and saved exactly one initial version. A visual copy review caught a partial-word substitution; the copy patcher now respects word boundaries. The follow-up NVIDIA edit returned HTTP 504 without changing the saved site. Explicit commands such as `Replace "Old heading" with "New heading" everywhere.` now use the same atomic patcher, responsive browser checks and version save without a model round trip. This path corrected the headings successfully and saved the second expected version. All three viewport checks passed again. General generation still depends on the free NVIDIA endpoint, which was slow and returned one 504 during testing. The direct UI review confirmed FAQ expansion, CTA section navigation, and mobile-menu open/close. The corrected React archive contains eight parseable code files. Both HTML and interaction states were checked for the corrected heading. The remote worker is not yet connected to Vercel.
