...
test(
  'Vercel gateway → separate worker → real browser checks → durable save, including restart and retry',
  { timeout: 120000 },
  async (t) => {
    ...
    await writeFile(path.join(jobPath(previousId), 'reference-1440.png'), png);
    ...
    assert.equal((await read(projectKey(owner, projectId))).versions.length, 1);
